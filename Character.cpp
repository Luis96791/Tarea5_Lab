...
Move* Character::getMove(SDL_Renderer *renderer, string name, int sprite_amount)
{
    ...
    for(int i=1;i<=sprite_amount;i++)
    {
        ...
        vector<Hitbox*>hitboxes;//vector hitboxes rojas
        vector<Hitbox2*>hitboxes2;//vector hitboxes azules
        ...
        //leer hitboxes azules
        string hitbox_file_path1 = "assets/" + name + "/hitboxes2/"+toString(i)+".txt";
        ifstream hitbox_file1(hitbox_file_path1.c_str());

        int x1,y1,w1,h1;
        while(hitbox_file1>>x1)
        {
            hitbox_file1>>y1;
            hitbox_file1>>w1;
            hitbox_file1>>h1;
            hitboxes2.push_back(new Hitbox2(x1,y1,w1,h1));
        }

        sprites.push_back(new Sprite(renderer,path,10,align_x,align_y,hitboxes,hitboxes2));
    }
